    ...
    <section id="pricing" className="py-24 bg-muted/30">
      <div className="container mx-auto px-4">
        <div className="text-center mb-10">
          ...
          <p className="text-xl text-muted-foreground">Choose a monthly plan. LTD for early adopters.</p>
        </div>
          ...
          {plans.map((plan) => (
            <div
              ...
              className={`relative gradient-card rounded-2xl border p-8 transition-smooth ${
                plan.popular ? "border-primary/70 hover:shadow-glow" : "border-border hover:shadow-lg"
              }`}
            >
              {plan.popular && (
              ...
              <Button variant={plan.popular ? "hero" : "secondary"} className="w-full">{plan.cta}</Button>
            </div>
          ))}
        </div>
        ...
        <div className="max-w-4xl mx-auto mb-10">
          <p className="text-center text-xs text-muted-foreground">
            Overage <strong>€3 / 1k steps</strong>. Governance features (Plan & Diff, Approvals, Undo) included on all plans.
          </p>
        </div>
        ...
        <div className="max-w-6xl mx-auto mb-12">
          ...
          <div className="flex flex-wrap justify-center gap-2">
            {addons.map((a) => (
              <span key={a} className="px-3 py-1.5 bg-card border border-border rounded-lg text-xs">{a}</span>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto">
          <div id={ltd.id} className="relative gradient-card rounded-2xl border border-primary p-8 shadow-glow">
            <div className="absolute -top-3 left-4">
              <Badge className="shadow" variant="default">Early Adopter</Badge>
            </div>
            ...
            <Button variant="hero" className="w-full">{ltd.cta}</Button>
            <p className="mt-3 text-xs text-muted-foreground leading-relaxed">
              LTDs are non-transferable; designed for early adopters, not high-volume production. Feature parity follows Starter/Pro; enterprise features (Team, Private Runner, SSO/SCIM, Audit export) are not included in LTD. Upgrade to monthly anytime — LTD benefits remain for the included limits.
            </p>
          </div>
        </div>
      </div>
