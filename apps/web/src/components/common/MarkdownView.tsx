...
export default function MarkdownView({ content }: { content: string }) {
  const components: Components = {
    ...
    p: ({ children }) => (
      <p className="leading-relaxed mb-4 text-gray-600 dark:text-gray-400">{children}</p>
    ),
    a: ({ href, children }) => (
      <a
        href={href}
    ...
    strong: ({ children }) => (
      <strong className="text-gray-800 dark:text-gray-200 font-semibold">{children}</strong>
    ),
    ...
    li: ({ children }) => (
      <li className="text-gray-600 dark:text-gray-400 leading-relaxed">{children}</li>
    ),
    blockquote: ({ children }) => (
    ...
    table: ({ children }) => <table className="border-collapse w-full my-6">{children}</table>,
    th: ({ children }) => (
