...
export function createGraphEventHandler(opts: {
    ...
    const { run, liveStateRef, markStep, applyDelta, safePublish, stepLogs, setGraphEmitted, logger, telemetry } =
        opts;
        ...
        try {
            switch (evt.type) {
                ...
                case 'tool.succeeded': {
                    ...
                    void telemetry.track('step_succeeded', { runId: run.id, tool: name }).catch(() => undefined);
                    break;
                }
                case 'tool.failed': {
                    const name = (evt.payload as any)?.name ?? 'unknown';
