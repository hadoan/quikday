...
function validateExtraction(
  ...
  const expectedWords = expected.expectedOutcome
    .toLowerCase()
    .split(' ')
    .filter(
      (word) =>
        word.length > 3 && !['about', 'with', 'from', 'that', 'this', 'should'].includes(word),
    );
...
async function runAllTests() {
  ...
  for (let i = 0; i < GOLDEN_UTTERANCES.length; i++) {
    ...
    console.log(
      `  Expected Missing Fields: ${utterance.expectedMissing.length > 0 ? utterance.expectedMissing.join(', ') : 'none'}`,
    );
    console.log();
    ...
    if (result.success) {
      ...
      console.log(
        `  Missing Fields: ${result.goal.missing?.length > 0 ? result.goal.missing.join(', ') : 'none'}`,
      );
      console.log(
        `  Prompt Length: ${result.promptLength} chars (~${Math.ceil(result.promptLength / 4)} tokens)`,
      );
      console.log();
      ...
      if (!v.domainMatch) {
        console.log(
          `    (Expected: ${utterance.domains.join(', ')} / Got: ${result.domains.join(', ')})`,
        );
      }
  ...
  console.log(
    `✅ Valid Extractions: ${successCount} (${((successCount / GOLDEN_UTTERANCES.length) * 100).toFixed(1)}%)`,
  );
  console.log(
    `❌ Invalid/Failed: ${failCount} (${((failCount / GOLDEN_UTTERANCES.length) * 100).toFixed(1)}%)`,
  );
  console.log();
  ...
  if (successfulResults.length > 0) {
    ...
    const avgPromptLength =
      successfulResults.reduce((sum, r) => sum + r.promptLength, 0) / successfulResults.length;
    const avgConfidence =
      successfulResults.reduce((sum, r) => sum + (r.goal?.confidence || 0), 0) /
      successfulResults.length;

    console.log('Performance Metrics:');
    console.log(`  Avg Duration: ${avgDuration.toFixed(0)}ms`);
    console.log(
      `  Avg Prompt Length: ${avgPromptLength.toFixed(0)} chars (~${Math.ceil(avgPromptLength / 4)} tokens)`,
    );
    ...
    console.log(
      `  Outcome Accuracy: ${outcomePassCount}/${validatedResults.length} (${((outcomePassCount / validatedResults.length) * 100).toFixed(1)}%)`,
    );
    console.log(
      `  Domain Accuracy: ${domainPassCount}/${validatedResults.length} (${((domainPassCount / validatedResults.length) * 100).toFixed(1)}%)`,
    );
    console.log(
      `  Confidence Threshold: ${confidencePassCount}/${validatedResults.length} (${((confidencePassCount / validatedResults.length) * 100).toFixed(1)}%)`,
    );
    console.log();
  }
  ...
  if (failCount > 0) {
    ...
    results.forEach((r) => {
      if (!r.success) {
        ...
        if (!r.validation.domainMatch)
          console.log(
            `      ❌ Domain mismatch (expected: ${GOLDEN_UTTERANCES.find((u) => u.id === r.id)?.domains.join(', ')}, got: ${r.domains.join(', ')})`,
          );
        if (!r.validation.confidenceOk)
          console.log(`      ❌ Low confidence (${r.goal?.confidence})`);
      }
    });
    console.log();
  }
