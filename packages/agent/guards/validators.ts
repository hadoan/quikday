 ...
 */
export function validateDateTime(input: string, timezone: string = 'UTC'): { 
  valid: boolean; 
  iso?: string; 
  error?: string;
  ...
  const relativePatterns = /tomorrow|today|next\s+week|next\s+month|in\s+\d+\s+(days?|hours?|minutes?)/i;
 ...
 */
export function validateDuration(input: unknown): { valid: boolean; minutes?: number; error?: string } {
  if (typeof input === 'number') {
    if (input > 0 && input <= 1440) { // Max 24 hours
...
export function filterIntegrationPolicyQuestions(
  ...
  return missing.filter(item => {
    ...
    return !Array.from(forbiddenKeys).some(forbidden => keyLower.includes(forbidden.toLowerCase()));
  });
}
