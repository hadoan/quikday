...
const resolveWhen = (s: RunState) => {
  const start =
    (s.scratch as any)?.when?.startISO ??
    (s.scratch as any)?.schedule?.start ??
    null;
  const end =
    (s.scratch as any)?.when?.endISO ??
    (s.scratch as any)?.schedule?.end ??
    null;
  return { start, end };
};
...
function finalizeSteps(steps: Omit<PlanStep, "id" | "risk" | "dependsOn">[]): PlanStep[] {
  return steps.map((st, i) => {
    ...
    const risk =
      st.tool === "calendar.createEvent" || st.tool.endsWith("_write") ? "high" : "low";
...
async function callLLM(s: RunState, system: string, user: string): Promise<string | null> {
  const llm = (s as any)?.services?.llm as
    | { complete: (opts: { system: string; user: string; temperature?: number; response_format?: any }) => Promise<string> }
    | undefined;
  ...
  const payload = {
    ...
    samples: [
      ...
      },
      {
        ...
        expected_output: {
          steps: [
            { tool: "calendar.checkAvailability", args: { start: "2025-11-02T09:00:00Z", end: "2025-11-02T09:30:00Z" } },
            { tool: "calendar.createEvent", args: { title: "Candidate Intro", start: "2025-11-02T09:00:00Z", end: "2025-11-02T09:30:00Z", notifyAttendees: true } }
          ]
        }
      },
      {
        ...
        expected_output: {
          ...
          questions: [
            { key: "when.startISO", question: "What start time should I use for the meeting?", rationale: "Required to check availability and create the event." },
      ...
      },
      {
        ...
        input: {
          intent: "email.send",
          context: { subject: "Quick intro", toPreview: ["****"], summary: "Short intro about the role." },
          allowedTools: ["email.send"]
        },
        expected_output: {
          steps: [
            { tool: "email.send", args: { subject: "Quick intro", to: ["****"], body: "Short intro about the role." } }
          ]
        }
      }
    ]
...
function patchAndHardenPlan(s: RunState, drafted: z.infer<typeof PlanInSchema>): {
  steps: PlanStep[];
...
export const planner: Node<RunState> = async (s) => {
  ...
  // Emit preview
  events.planReady(s, safe(steps ?? []), diff);
