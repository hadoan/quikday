export const PLANNER_SYSTEM = [
  ...
  '- Return ONLY raw JSON. No explanations, no text before or after the JSON.',
  '- Do NOT include any markdown code fences (```json or ```).',
  '- Do NOT add any commentary, reasoning, or explanation.',
  ...
  '- NEVER use chat.respond to ask for missing information',
  '- When required arguments are missing, use null or empty string in the args object',
  '- The system will automatically detect missing inputs and prompt the user',
  '- Your job is to plan the ACTION, not to ask questions about missing data',
  '- Example: Goal is "Send email to user@example.com" but subject/body missing → plan email.send with null values',
  '- Example: Goal is "Set out-of-office" but dates missing → plan email.setOutOfOffice with null values',
  '',
  ...
  "- Even when required values are missing (null/empty), ALWAYS plan the ACTION step - don't fall back to chat.respond",
  '',
  ...
  '',
  'Global rules:',
  '- CRITICAL: Output ONLY a single JSON object matching the schema above. NO TEXT BEFORE OR AFTER.',
  '- CRITICAL: Return RAW JSON only. Do NOT wrap output in Markdown code fences or backticks.',
  '- CRITICAL: Do NOT prefix with ```json; no code blocks; no explanations — ONLY JSON.',
  ...
  '- CRITICAL: All string values in JSON MUST properly escape special characters:',
  '  • Newlines → \\n (not literal line breaks)',
  '  • Tabs → \\t',
  '  • Quotes → \\"',
  '  • Backslashes → \\\\',
  '  • Example CORRECT: "body": "Line 1\\n\\nLine 2\\n\\nBest regards"',
  '  • Example WRONG: "body": "Line 1\n\nLine 2\n\nBest regards" (literal newlines are invalid JSON)',
  '-',
  ...
  '- "Send [X] to [person/email]" → ALWAYS use email.send (NOT chat.respond), even if subject/body missing',
  '- "Draft [X] to [person/email]" → use email.draft.create, even if content missing',
  ...
  '- CRITICAL: Use null for missing email content (subject, body) - NEVER use chat.respond to ask for them',
  '- Use null or empty string for missing email addresses - they will be requested from the user',
  '- DO NOT use chat.respond as fallback for ANY email operations',
  '',
...
export function buildPlannerSystemPrompt(
  ...
  const examples = [
    ...
    '// NOTE: String values with newlines must use \\n escape sequences, not literal newlines',
    '{',
    '  "steps": [',
