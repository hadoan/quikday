...
export function buildPlannerSystemPrompt(tools: Array<{ name: string; description: string; args: any }>): string {
  ...
  const examples = [
    ...
    '}',
    '',
    // Email followup - multi-step workflow with placeholders
    '// For "No-Reply Sweep" or "Find emails without replies":',
    '// Search → Generate drafts for each → Send approved',
    '// If inputs are: { "days_ago": 7, "max_results": 10, "tone": "polite" }',
    '{',
    '  "steps": [',
