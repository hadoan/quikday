...
export const CALENDAR_DOMAIN_RULES_V1 = [
  ...
  '- For scheduling: may involve attendees, duration, preferred_times, meeting_title',
  '- Meeting duration should be in minutes (convert from natural language)',
  '- Attendees must be valid email addresses',
  '- Time zones matter: capture user\'s implied time zone when provided',
  '- For availability checks: time window and duration are typically needed',
].join('\n');
