...
export class GmailEmailService implements EmailService {
    ...
    async search(q: SearchQuery) {
        ...
        return { messages: results, nextPageToken: typeof listing?.nextPageToken === 'string' ? listing.nextPageToken : undefined };
    }

    async createDraft(draft: DraftInput) {
        this.logger.log(this.formatMeta({ op: 'createDraft', toCount: draft.to?.length ?? 0, subject: draft.subject }));
        ...
        const resp = await gmail.users.drafts.create({ userId: 'me', requestBody: { message: { raw: rawMessage } } });
        const data: any = resp.data;
        const draftId = typeof data?.id === 'string' ? data.id : '';
        const threadId = typeof data?.message?.threadId === 'string' ? data.message.threadId : undefined;
    ...
    async changeLabels(target: { threadId?: string; messageId?: string }, delta: LabelChange) {
        ...
        const toIds = (names: string[]) => names
        ...
        if (target.threadId) {
            await gmail.users.threads.modify({ userId: 'me', id: target.threadId, requestBody: { addLabelIds, removeLabelIds } });
            return;
        }

        if (target.messageId) {
            await gmail.users.messages.modify({ userId: 'me', id: target.messageId, requestBody: { addLabelIds, removeLabelIds } });
            return;
        }
    ...
    private async resolveAccessContext(): Promise<{ accessToken: string; refreshToken?: string | null; expiresAt?: Date; credentialId: number; email?: string }>
    {
        const currentUserId = this.currentUserService.getCurrentUserId();
        if (!currentUserId) throw new Error('No current user in context');
        ...
        const email = typeof credential.emailOrUserName === 'string' && credential.emailOrUserName.length > 0
            ? credential.emailOrUserName
            : undefined;
        return { accessToken, refreshToken, expiresAt, credentialId: credential.id, email };
    }

    private async getGmailClient(): Promise<{ gmail: gmail_v1.Gmail; credentialId: number; email?: string }>
    {
        const { accessToken, refreshToken, expiresAt, credentialId, email } = await this.resolveAccessContext();
        const { clientId, clientSecret } = await this.getOAuthCredentials();
        const oAuth2 = new google.auth.OAuth2(clientId, clientSecret);
        oAuth2.setCredentials({ access_token: accessToken, refresh_token: refreshToken ?? undefined, expiry_date: expiresAt?.getTime() });
    ...
    private async getOAuthCredentials(): Promise<OAuthCredentials> {
        ...
        this.logger.debug?.(this.formatMeta({ op: 'getOAuthCredentials', clientIdPresent: !!clientId, clientSecretPresent: !!clientSecret }));
    ...
    private async sendEmailViaGmailApi(_userId: string, rawMessage: string, _accessToken: string): Promise<GmailSendResponse> {
        this.logger.log(this.formatMeta({ op: 'sendEmailViaGmailApi' }));
        const { gmail } = await this.getGmailClient();
        const resp = await gmail.users.messages.send({ userId: 'me', requestBody: { raw: rawMessage } });
    ...
    private extractHeader(headers: any[], name: string): string | undefined {
        const h = headers?.find?.((x: any) => typeof x?.name === 'string' && x.name.toLowerCase() === name.toLowerCase());
    ...
    private mapGmailMessageToEmailMessage(m: any): EmailMessage {
        ...
        const labelIds: string[] = Array.isArray(m?.labelIds) ? m.labelIds.filter((x: any) => typeof x === 'string') : [];
        ...
        return {
            ...
            headers: headers?.reduce?.((acc: Record<string, string>, h: any) => {
    ...
    private async fetchLabelsMap(): Promise<{ idMap: Map<string, string>; nameMap: Map<string, string> }>
    {
