...
export default function createApp(meta: AppMeta, deps: any) {
  return new (class GmailApp {
      ...
      try {
        ...
        if (typeof this.deps?.createSignedState === 'function') {
          try {
            ...
            if (userId) {
              signedState = this.deps.createSignedState({ userId, timestamp: Date.now(), returnTo: req.query?.returnTo });
            }
          } catch (stateError) {
            console.warn('✉️  [Add] Failed to create signed state', { error: stateError instanceof Error ? stateError.message : 'Unknown' });
          }
        }
    ...
    async post(req: any, res: any) {
      ...
      const now = Date.now();
      return res.status(200).json({ ok: true, app: meta.slug, variant: meta.variant, received: body, timestamp: now });
    }
  })(meta, deps);
}
